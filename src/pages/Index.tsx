    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="relative overflow-hidden">
        <div className="absolute inset-0">
          <img src={heroBanner} alt="बनारसी साड़ी" className="w-full h-full object-cover" />
        ...
        <div className="relative container mx-auto px-4 py-20 md:py-32">
          ...
          >
            <h1 className="text-3xl md:text-5xl font-extrabold text-primary-foreground leading-tight">
              ...
              <br />
              <span className="text-2xl md:text-3xl font-semibold opacity-90">वॉइस-फर्स्ट साड़ी इंडस्ट्री नेटवर्क</span>
            </h1>
            <p className="mt-4 text-primary-foreground/80 text-base md:text-lg max-w-lg">
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Link to="/register" className="inline-flex items-center gap-2 px-6 py-3 gradient-hero text-primary-foreground font-semibold rounded-xl shadow-warm hover:scale-105 transition-transform">
                नया पंजीकरण
              </Link>
              <Link to="/search" className="inline-flex items-center gap-2 px-6 py-3 bg-primary-foreground/10 backdrop-blur text-primary-foreground font-semibold rounded-xl border border-primary-foreground/20 hover:bg-primary-foreground/20 transition-colors">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {features.map((f, i) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="bg-card p-6 rounded-xl shadow-card border border-border hover:shadow-elevated hover:-translate-y-1 transition-all"
            >
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {roles.map(([key, label]) => (
              <Link
                key={key}
                to={`/search?role=${key}`}
                className="px-5 py-2.5 rounded-full bg-primary/10 text-primary font-medium text-sm hover:bg-primary hover:text-primary-foreground transition-colors"
              >
                {label}
              </Link>
            ))}
      ...
      <section className="container mx-auto px-4 py-16 text-center">
        <h2 className="text-2xl font-bold mb-3">अभी जुड़ें SareeSetu से</h2>
        <p className="text-muted-foreground mb-6">वाराणसी की साड़ी इंडस्ट्री को डिजिटल बनाएँ</p>
